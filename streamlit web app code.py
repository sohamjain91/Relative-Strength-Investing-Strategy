...
input_path = r"C:\Users\soham\relative strength strategy\final_datas"
...
if option == 'Sectors Analysis':
    ...
    styled_df = sector_returns_df.style.background_gradient(cmap=cmap)#, subset=['Col1', 'Col2', 'Col3'], vmin=-5, vmax=25)
    st.write(styled_df)
...
if option == 'Sector Indices':
    ...
    df_final_stocks = final_df[final_df['sector'] == sectors_select[0]].sort_values(by='RS').nlargest(number_of_stock, columns='RS').reset_index(drop=True)
    ...
    for i, sector in enumerate(sectors_select[1:], start=1):
        df_final_stocks1 = final_df[final_df['sector'] == sector].sort_values(by='RS').nlargest(number_of_stock, columns='RS').reset_index(drop=True)
